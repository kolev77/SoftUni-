...
public class Main {
    ...
    private static Animal createAnimal(String[] tokens) {
        ...
        switch (tokens[0]) {
            case "Cat":
                return new Cat(tokens[1], tokens[0], Double.valueOf(tokens[2]), 0, tokens[3], tokens[4]);
            case "Tiger":
                return new Tiger(tokens[1], tokens[0], Double.valueOf(tokens[2]), 0, tokens[3]);
            case "Zebra":
                return new Zebra(tokens[1], tokens[0], Double.valueOf(tokens[2]), 0, tokens[3]);
            case "Mouse":
                return new Mouse(tokens[1], tokens[0], Double.valueOf(tokens[2]), 0, tokens[3]);
            default:
                return null;
        }
    }
