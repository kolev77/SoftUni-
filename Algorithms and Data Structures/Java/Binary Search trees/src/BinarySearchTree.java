...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    public void delete(T key) {
        ...
        while (forDeleting != null) {
            ...
            if (forDeleting.getValue() == key) break;
    ...
    public int rank(T item) {
        return this.getRankOf(this.root, item);
    }

    private int getRankOf(Node node, T item) {
        if (node == null) {
            return 0;
        }

        if (node.value.compareTo(item) > 0) {   // if the root`s value is bigger than input value
            return getRankOf(node.left, item);   // we go to the left part of our tree
        } else if (node.value.compareTo(item) < 0) { // if the root`s value is smaller than input value
            return 1 + this.getChildrenCount(node.left) + getRankOf(node.getRight(), item);
        }
