...
public class ServerConfig {
    ...
    private static final String CONFIG_FILE_FULL_PATH =
            WebConstants.WEB_SERVER_ROOT_FOLDER_PATH + CONFIG_FILE_NAME;

    LinkedHashSet<String> requestHandlersPriority;
    ...
    private void loadRequestHandlerConfig(String configFileLine) {
        List<String> requestHandlerConfigParams = Arrays.asList(configFileLine
                .split("[\\s\\,]"))
                .stream().filter(x -> x.length() > 0).collect(Collectors.toList());
    ...
    public void loadConfig() {
        try {
            List<String> configFileContents = Files.readAllLines(Paths.get(CONFIG_FILE_FULL_PATH));
