...
abstract class BaseDynamicHandler extends BaseHandler {
    ...
    private static final String BASE_VIEW_FULL_PATH = System.getProperty("user.dir")
            + "\\src\\org\\softuni\\main\\casebook\\resources\\base-template.html";

    private static final String HEADER_VIEW_FULL_PATH = System.getProperty("user.dir")
    ...
    protected User getCurrentUser(HttpRequest request, UserRepository userRepository) {
        ...
        String userId = this.httpSessionStorage.getSession(cookie.getValue())
                .getAttributes()
                .get("user-id")
                .toString();
    ...
    private String getBaseView() {
        try {
            List<String> content = Files.readAllLines(Paths.get(BASE_VIEW_FULL_PATH));
    ...
    private String getHeaderView(String view) {
        try {
            List<String> content = Files.readAllLines(Paths.get(HEADER_VIEW_FULL_PATH + view + VIEWS_EXTENSION));
            return String.join("", content);
        } catch (IOException e) {
            return null;
        }
    ...
    private String getView(String viewName) {
        try {
            List<String> content = Files.readAllLines(Paths.get(VIEWS_FULL_PATH + viewName + VIEWS_EXTENSION));
            return String.join("", content);
        } catch (IOException e) {
            return null;
        }
